...
const AddEmployee = () => {
  ...
  const [merchandising_departments, setMerchandisingDepartments] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [compRes, custRes, deptRes, mdeptRes] = await Promise.all([
          getCompanies(),
          getCustomers(),
          getDepartments(),
          getMDepartments(),
        ...
        setMerchandisingDepartments(mdeptRes.data || []);
      } catch (err) {
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !/^[0-9+\-\s()]{10,15}$/.test(formData.personal_phone.trim())
    ) {
      newErrors.personal_phone = "Invalid phone number (10-15 digits)";
    }
    if (
      formData.office_phone?.trim() &&
      !/^[0-9+\-\s()]{10,15}$/.test(formData.office_phone.trim())
    ) {
    ...
    if (
      formData.salary_cash &&
      (isNaN(formData.salary_cash) || Number(formData.salary_cash) < 0)
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Handle merchandising_department (multi-select array)
      if (
        Array.isArray(formData.merchandising_department) &&
        formData.merchandising_department.length > 0
      ) {
        const validDeptIds = formData.merchandising_department
          .map((id) => parseInt(id, 10))
          .filter((id) => !isNaN(id) && id > 0);

        validDeptIds.forEach((id) => {
          formDataToSend.append("merchandising_department", id);
        });
      } else if (
        typeof formData.merchandising_department === "string" &&
        formData.merchandising_department
      ) {
        const id = parseInt(formData.merchandising_department, 10);
        if (!isNaN(id) && id > 0) {
          formDataToSend.append("merchandising_department", id);
        }
      }
      ...
      if (
        formData.salary_cash &&
        !isNaN(formData.salary_cash) &&
        Number(formData.salary_cash) > 0
      ) {
      ...
      if (error.response?.data) {
        ...
        if (error.response.status === 400) {
          if (errorData.email && errorData.email.includes("already exists")) {
            ...
            errorData.employee_id.includes("already exists")
          ) {
            errorMessage =
              "This Employee ID is already taken. Please use a different ID.";
            setErrors((prev) => ({
              ...prev,
  ...
  const formSections = [
    ...
    },
    {
      ...
      fields: [
        ...
        {
          name: "merchandising_department",
          label: "Management Departments",
          type: "select",
          multiple: true, // Add this
          options: [
            { label: "Select Management Departments", value: "" },
    ...
    },
    {
      ...
      icon: <FaImage />,
      fields: [{ name: "image1", label: "Upload Photo", type: "file" }],
    },
  ];
    ...
    <div className="add-employee-container">
      ...
      <div className="content-wrapper">
        <div className="add-employee-card">
          ...
          {showPopup && (
            <div
              className={`success-popup ${successMessage.includes("Failed") ? "error-popup" : ""}`}
            >
                        ...
                        {field.type === "select" ? (
                          <>
                            <div className="select-with-icon">
                              <select
                                ...
                                multiple={field.multiple || false} // Add this
                                size={field.multiple ? 4 : 1} // Show multiple options
                              >
                          ...
                          <div className="customers-section">
                            <div className="customers-grid">
                              {(customers || []).length > 0 ? (
                                customers.map((c) => {
                                  const checked = formData.customer.includes(
                                    c.id?.toString(),
                                  );
                                  return (
                                    <label
                                    ...
                                    >
                                      <input
                                        ...
                                        checked={checked}
                                        onChange={() =>
                                          handleCheckboxChange(c.id?.toString())
                                        }
                                        disabled={isLoading}
                                      />
