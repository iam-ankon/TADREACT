...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const hybridUsers = ["ZOHAER"]; // Users who have both full access AND team leader view

      console.log("🔍 Current Username:", currentUsername);
      console.log("🔍 Is Hybrid User:", hybridUsers.includes(currentUsername));

      // Get current user's leaves (always show)
      ...
      // For HYBRID users (ZOHAER) - show team leaves on dashboard (like team leader)
      if (hybridUsers.includes(currentUsername)) {
        console.log("👤 Hybrid user - showing team leaves on dashboard");
        try {
          const teamLeavesResponse = await getTeamLeaves();
          if (
            teamLeavesResponse.data &&
            Array.isArray(teamLeavesResponse.data)
          ) {
            teamLeavesData = teamLeavesResponse.data;
          } else if (
            teamLeavesResponse.data?.data &&
            Array.isArray(teamLeavesResponse.data.data)
          ) {
            teamLeavesData = teamLeavesResponse.data.data;
          }

          // Filter out own leaves
          const currentEmployeeId = employeeInfo.employee_id;
          teamLeavesData = teamLeavesData.filter((leave) => {
            const leaveEmployeeId =
              leave.employee_code ||
              leave.employee?.employee_id ||
              leave.employee_id;
            return leaveEmployeeId !== currentEmployeeId;
          });
      ...
      // For regular FULL ACCESS users (not hybrid) - don't show team leaves on dashboard
      else if (
        ...
        console.log(
          "👑 Full access user - not showing team leaves on dashboard",
        );
        teamLeavesData = [];
      } else {
        // Regular team leaders - show team leaves
        const hasReportingResponsibilities =
  ...
  const LeaveItem = ({ leave, isTeamLeave = false }) => {
    ...
    const leaveEmployeeName =
      leave.employee_name || leave.employee?.name || "Unknown";
    ...
    return (
      <div
        ...
        onMouseEnter={(e) => {
          if (leave.id)
            e.currentTarget.style.backgroundColor = isTeamLeave
              ? "#e3f2fd"
              : "#f8f9fa";
        }}
        onMouseLeave={(e) => {
      ...
      >
        <div style={styles.leaveHeader}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <h4 style={styles.leaveType}>
              ...
              {isTeamLeave && (
                <span style={styles.teamLeaveBadge}>Team Member</span>
              )}
            </h4>
          </div>
        ...
        <p style={styles.leaveDetails}>
          <strong>Leave Period:</strong> {formatDate(leave.start_date)} to{" "}
          {formatDate(leave.end_date)}
        </p>

        <p style={styles.leaveDetails}>
          <strong>Duration:</strong> {leave.leave_days || "N/A"} day(s)
        </p>
        ...
        <p style={styles.debugInfo}>
          Leave ID: {leave.id} | Employee: {leaveEmployeeName} | Type:{" "}
          {leave.leave_type} | Team Leave: {isTeamLeave ? "Yes" : "No"}
        </p>
      </div>
    );
  };
        ...
        <div style={styles.header}>
          <div style={styles.headerContent}>
            <div style={styles.headerText}>
              ...
              <p style={styles.debugInfo}>
                Employee ID: {employeeInfo.employee_id} | My Leaves:{" "}
                {myLeaves.length} | Team Leaves: {teamLeaves.length} | Last
