...
const HRWorkPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchInterviews = async () => {
      try {
        const response = await axios.get('http://119.148.12.1:8000/api/hrms/api/interviews/');
    ...
    const fetchLeaveRequests = async () => {
      try {
        const response = await axios.get('http://119.148.12.1:8000/api/hrms/api/employee_leaves/');
    ...
    const fetchAttendanceData = async () => {
      try {
        const response = await axios.get('http://119.148.12.1:8000/api/hrms/api/attendance/');
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', backgroundColor: '#DCEEF3', }}>
      {/* Sidebar */}
