...
const EmployeeDetails = () => {
  ...
  // NEW: Joining Date Filter state
  const [joiningDateFrom, setJoiningDateFrom] = useState(() => {
    try {
      return localStorage.getItem("employeeJoiningDateFrom") || "";
    } catch {
      return "";
    }
  });
  const [joiningDateTo, setJoiningDateTo] = useState(() => {
    try {
      return localStorage.getItem("employeeJoiningDateTo") || "";
    } catch {
      return "";
    }
  });
  ...
  const joiningDateDropdownRef = useRef(null);
  ...
  const buildFilterParams = useCallback(() => {
    ...
    if (birthdateFilter) {
      ...
      params.birth_month = parseInt(month, 10);
  ...
  useEffect(() => {
    ...
    filterTimeoutRef.current = setTimeout(() => {
      try {
        ...
        localStorage.setItem("employeeJoiningDateFrom", joiningDateFrom);
        localStorage.setItem("employeeJoiningDateTo", joiningDateTo);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      const isOutsideJoiningDate =
        joiningDateDropdownRef.current &&
        !joiningDateDropdownRef.current.contains(event.target);

      if (isOutsideDesignation && isOutsideDepartment && isOutsideBirthdate && isOutsideJoiningDate) {
        closeDropdowns();
      }
    };
  ...
  const handleJoiningDateFromChange = (date) => {
    setJoiningDateFrom(date);
  };

  const handleJoiningDateToChange = (date) => {
    setJoiningDateTo(date);
  };

  const clearJoiningDateFrom = () => {
    setJoiningDateFrom("");
  };

  const clearJoiningDateTo = () => {
    setJoiningDateTo("");
  };

  const clearAllJoiningDates = () => {
    setJoiningDateFrom("");
    setJoiningDateTo("");
  };
        ...
        <div style={styles.employeeDashboard}>
          ...
          <div style={styles.filtersSection}>
            ...
            <div style={styles.filtersGrid}>
              ...
              {/* Joining Date Filter */}
              <div style={styles.filterWrapper} ref={joiningDateDropdownRef}>
                <div
                  style={{
                    ...styles.filterSelect,
                    ...(showJoiningDatePicker ? styles.filterSelectActive : {}),
                  }}
                  onClick={() => {
                    setShowJoiningDatePicker(!showJoiningDatePicker);
                    setShowDesignationDropdown(false);
                    setShowDepartmentDropdown(false);
                    setShowBirthdatePicker(false);
                  }}
                >
                  <span>
                    {joiningDateFrom || joiningDateTo ? (
                      <span style={styles.dateRangeDisplay}>
                        {joiningDateFrom && (
                          <span>From: {formatDateForDisplay(joiningDateFrom)}</span>
                        )}
                        {joiningDateFrom && joiningDateTo && " | "}
                        {joiningDateTo && (
                          <span>To: {formatDateForDisplay(joiningDateTo)}</span>
                        )}
                      </span>
                    ) : (
                      <span style={styles.placeholder}>Joining Date Range</span>
                    )}
                  </span>
                ...
                {showJoiningDatePicker && (
                  <div style={{ ...styles.dropdownMenu, ...styles.datePickerDouble }}>
                    <div style={styles.dateRangeContainer}>
                      <div style={styles.dateRangeColumn}>
                        <div style={styles.dateRangeLabel}>From Date</div>
                        <input
                          type="date"
                          value={joiningDateFrom}
                          onChange={(e) => handleJoiningDateFromChange(e.target.value)}
                          style={styles.dateInput}
                          placeholder="Start Date"
                        />
                      </div>
                      <div style={styles.dateRangeColumn}>
                        <div style={styles.dateRangeLabel}>To Date</div>
                        <input
                          type="date"
                          value={joiningDateTo}
                          onChange={(e) => handleJoiningDateToChange(e.target.value)}
                          style={styles.dateInput}
                          placeholder="End Date"
                        />
                      </div>
                    </div>
                    <div style={styles.dateRangeActions}>
                      <button
                      ...
                      <button
                        ...
                        onClick={() => setShowJoiningDatePicker(false)}
                      >
                        Apply
                      </button>
                    </div>
            ...
            {/* Birthdate Filter - Second Row */}
            <div style={styles.filtersGridSecondRow}>
            ...
            {/* Active Filters */}
            {(designationFilter || departmentFilter || birthdateFilter || joiningDateFrom || joiningDateTo) && (
              <div style={styles.activeFilters}>
                ...
                {joiningDateFrom && (
                  <span style={styles.filterTag}>
                    Joining From: {formatDateForDisplay(joiningDateFrom)}
                    <button
                      style={styles.filterTagButton}
                      onClick={() => setJoiningDateFrom("")}
                    >
                      <FaTimes />
                    </button>
                  </span>
                ...
                {joiningDateTo && (
                  <span style={styles.filterTag}>
                    Joining To: {formatDateForDisplay(joiningDateTo)}
                    <button
                      style={styles.filterTagButton}
                      onClick={() => setJoiningDateTo("")}
                    >
                      <FaTimes />
                    </button>
                  </span>
                  ...
                  {employees.length > 0 ? (
                    employees.map((employee) => (
                      ...
                      >
                        ...
                        <td style={styles.tableCell}>
                          <div style={styles.joiningDateInfo}>
                            <FaCalendarAlt style={styles.icon} />
                            {formatDateForDisplay(employee.joining_date) || "—"}
                          </div>
                        </td>
                        <td style={styles.tableCell}>
