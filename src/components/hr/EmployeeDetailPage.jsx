...
import SalaryEmailModal from "./SalaryEmailModal";
import {
  FaEdit,
  FaArrowLeft,
  FaPrint,
...
const EmployeeDetailPage = () => {
  ...
  const [merchandisingDepartmentNames, setMerchandisingDepartmentNames] =
    useState([]);
  ...
  const [showSalaryEmailModal, setShowSalaryEmailModal] = useState(false);
  ...
  // Function to show confirmation dialog for welcome email
  ...
  // Fetch employee details - Updated to properly handle the data
  useEffect(() => {
    const fetchEmployeeDetails = async () => {
      try {
        ...
        // PRIORITY: Use merchandising_department_names from serializer (already has names)
        if (
          employeeData.merchandising_department_names &&
          Array.isArray(employeeData.merchandising_department_names) &&
          employeeData.merchandising_department_names.length > 0
        ) {
          console.log(
            "✅ Using merchandising_department_names:",
            employeeData.merchandising_department_names,
          );
          setMerchandisingDepartmentNames(
            employeeData.merchandising_department_names,
          );
        }
        // FALLBACK: Try to extract names from merchandising_department objects
        else if (employeeData.merchandising_department) {
          const deptNames = [];

          if (Array.isArray(employeeData.merchandising_department)) {
            employeeData.merchandising_department.forEach((dept) => {
            ...
            typeof employeeData.merchandising_department === "object" &&
            employeeData.merchandising_department !== null
          ) {
            const dept = employeeData.merchandising_department;
            const name =
              dept.merchandising_department_name ||
              dept.merchandising_department ||
              dept.name ||
          ...
          if (deptNames.length > 0) {
            console.log(
              "✅ Extracted names from merchandising_department:",
              deptNames,
            );
            setMerchandisingDepartmentNames(deptNames);
          } else {
            console.log(
              "⚠️ No names found in merchandising_department, showing IDs as fallback",
            );
            ...
            if (Array.isArray(employeeData.merchandising_department)) {
              employeeData.merchandising_department.forEach((dept) => {
                if (typeof dept === "object" && dept.id) {
                  ids.push(`Department ${dept.id}`);
                } else if (
                  typeof dept === "number" ||
                  typeof dept === "string"
                ) {
                  ids.push(`Department ${dept}`);
                }
              });
            }
            setMerchandisingDepartmentNames(ids);
          }
        }

        // Handle customer names
        if (
          employeeData.customer_names &&
          Array.isArray(employeeData.customer_names)
        ) {
          setCustomerNames(employeeData.customer_names);
        } else if (employeeData.customer) {
          // Fallback customer fetching logic
          const fetchCustomerNames = async () => {
            const names = [];
            const validCustomerIds = employeeData.customer.filter(
              (id) => id != null,
            );
          ...
          <div class="print-container">
            ...
            <div class="detail-sections">
              <div class="detail-section">
                ...
                <div class="detail-row">
                  <span>Management Departments:</span>
                  <span>${merchandisingDepartmentNames.length > 0 ? merchandisingDepartmentNames.join(", ") : "N/A"}</span>
                </div>
                <div class="detail-row">
                  <span>Customers:</span>
  ...
  return (
    <div className="employee-detail-container">
          ...
          <div className="scrollable-content">
            ...
            <div className="employee-header">
              ...
              <div className="action-buttons">
                ...
                <button
                  onClick={() => setShowSalaryEmailModal(true)}
                  className="btn-salary-email"
                >
                  <FaMoneyBillWave /> Send Salary Notification
                </button>
                <button
            ...
            <div id="printable-area" className="employee-content">
              ...
              <div className="profile-summary">
                ...
                <div className="profile-info">
                  ...
                  <div className="service-duration">
                    <FaStar />
                    <span>
                      Service: {calculateLengthOfService(employee.joining_date)}
                    </span>
                  </div>
                </div>
                <div className="salary-summary">
                  ...
                  <div className="salary-item">
                    ...
                    <span className="salary-amount">
                      {formatCurrency(employee.salary_cash)}
                    </span>
                  </div>
              ...
              <div className="detail-grid">
                {/* Company Information - Updated to show only names */}
                  ...
                  <div className="card-content">
                    <div className="info-row">
                      ...
                      <span className="info-value">
                        {employee.company_name || employee.company || "N/A"}
                      </span>
                    </div>
                    <div className="info-row">
                      <span className="info-label">Management Departments</span>
                      <span className="info-value">
                        {merchandisingDepartmentNames &&
                        merchandisingDepartmentNames.length > 0 ? (
                          <div className="management-depts">
                            {merchandisingDepartmentNames.map((name, idx) => (
                          ...
                          <span className="no-data">
                            No management departments assigned
                          </span>
                        )}
                      </span>
                    </div>
                      ...
                      <span className="info-value customers">
                        {customerNames.length > 0 ? (
                          customerNames.map((name, idx) => (
                            <span key={idx} className="customer-tag">
                              {name}
                            </span>
                          ))
                        ) : (
                ...
                <div className="detail-card">
                  ...
                  <div className="card-content">
                    ...
                    <div className="info-row">
                      ...
                      <span className="info-value">
                        {employee.personal_phone || "N/A"}
                      </span>
                    </div>
                    <div className="info-row">
                      <span className="info-label">Office Phone</span>
                      <span className="info-value">
                        {employee.office_phone || "N/A"}
                      </span>
                    </div>
                    <div className="info-row">
                    ...
                    <div className="info-row full-width">
                      ...
                      <span className="info-value">
                        {employee.special_skills ||
                          "No special skills recorded"}
                      </span>
                    </div>
                    <div className="info-row full-width">
                      <span className="info-label">Remarks</span>
                      <span className="info-value remarks">
                          ...
                          <div key={idx} className="increment-item">
                            ...
                            <div className="increment-amount">
                              <span className="from-amount">
                                {formatCurrency(inc.present_salary)}
                              </span>
                              <span className="increment-arrow">→</span>
                              <span className="to-amount">
                                {formatCurrency(inc.proposed_salary)}
                              </span>
                            </div>
                            {inc.remarks && (
                              <div className="increment-remarks">
                                {inc.remarks}
                              </div>
                            )}
                          </div>
          ...
          <div className="confirmation-modal">
            ...
            <div className="modal-body">
              ...
              <p className="modal-warning">
                This action will send welcome emails to the employee's
                registered email address and cannot be undone.
              </p>
            </div>
      ...
      {/* Salary Email Modal */}
      <SalaryEmailModal
        isOpen={showSalaryEmailModal}
        onClose={() => setShowSalaryEmailModal(false)}
        employee={employee}
        onSuccess={() => {
          console.log("Salary email sent successfully");
          setEmailStatus("✅ Salary notification sent successfully!");
          setTimeout(() => {
            setEmailStatus("");
          }, 5000);
        }}
