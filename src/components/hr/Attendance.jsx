...
const formatTime = (str) => {
  if (!str) return "00:00 AM";

  try {
    let [hours, minutes] = [0, 0];

    if (str.includes("T")) {
      // Full datetime format like "2025-07-10T09:30:00"
      const date = new Date(str);
      if (isNaN(date)) return "00:00 AM";
      hours = date.getHours();
      minutes = date.getMinutes();
    } else {
      // Time-only format like "09:30:00"
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      check_in: `${formData.check_in || "00:00"}:00`,
      check_out: formData.check_out ? `${formData.check_out}:00` : null,
      office_start_time: `${formData.office_start_time || "09:30"}:00`,
    };
    try {
      if (formData.id) await updateAttendance(formData.id, payload);
      else await addAttendance(payload);
  ...
  const handleEdit = (att) => {
    setFormData({
      ...
      check_in: att.check_in?.slice(11, 16) || "",
      check_out: att.check_out?.slice(11, 16) || "",
      office_start_time: att.office_start_time?.slice(11, 16) || "09:30",
    });
    setShowForm(true);
  };
      ...
      <div style={{ flex: 1, padding: "20px", overflowX: "auto" }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={tableStyle}>
            <thead>
              <tr>
                {["Employee", "Date", "Check In", "Check Out", "Office Start", "Actions"].map(
                  (head) => (
              ...
              {attendance
                ...
                .map((a) => (
                  <tr key={a.id}>
                    ...
                    <td style={tdStyle}>{formatTime(a.check_in)}</td>
                    <td style={tdStyle}>{formatTime(a.check_out)}</td>
                    <td style={tdStyle}>{formatTime(a.office_start_time)}</td>
                    <td style={tdStyle}>
                      <button
                      ...
                      <button
                        ...
                        style={{ ...actionButton, background: "#dc2626", marginLeft: "6px" }}
                      >
                        Delete
                      </button>
                    </td>
