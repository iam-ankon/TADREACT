...
const Attendance = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const attRes = await getAttendance(currentPage, itemsPerPage, {
          filters,
        });
  ...
  // ============================================================
  // 1. HELPER FUNCTIONS (defined first, don't depend on downloadCSV)
  // ============================================================
  ...
  const getEmployeeDetails = useCallback(
    (employeeId) => {
      ...
      const companyName =
        companyLookup[employee.company] || employee.company_name || "N/A";
  ...
  // ============================================================
  // 2. DOWNLOAD CSV FUNCTION (defined BEFORE functions that use it)
  // ============================================================
  ...
  // ============================================================
  // UPDATED: generateEmployeeFullReport - Fetches ALL data first
  // ============================================================

  const generateEmployeeFullReport = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `📊 Generating report for ${selectedEmployees.length} employees`,
      );
      ...
      // Date filter (if no month filter)
      if (!monthFilter && dateFilter) {
        filters.date = dateFilter;
      }

      // Date range filter (if no month or date filter)
      if (
        !monthFilter &&
        !dateFilter &&
        showDateRange &&
      ...
      // ============================================================
      // Fetch ALL attendance records for ALL employees first
      // (we'll filter client-side since API doesn't support employee_id)
      // ============================================================
      ...
      // ============================================================
      // Build a set of selected employee IDs for quick lookup
      // ============================================================
      ...
      selectedEmployees.forEach((emp) => {
        if (emp && emp.employee_id) {
          ...
          employeeNameMap[emp.employee_id] = emp.name || emp.employee_name;
        }
      });
      ...
      // ============================================================
      // Filter records to ONLY selected employees
      // ============================================================
      const filteredRecords = allRecords.filter((item) => {
        ...
        // Extract employee ID from the record
        if (typeof item.employee === "object" && item.employee !== null) {
          employeeId = item.employee.employee_id;
        } else if (typeof item.employee === "number") {
          // If it's a DB ID, look it up
          const empData = employeeLookup[item.employee];
          if (empData) {
        ...
        // Check if this employee is in our selected list
        return selectedEmployeeIds.has(employeeId);
      });
      ...
      if (filteredRecords.length === 0) {
        alert(
          `No attendance records found for the selected employees (${selectedEmployees.length} employees).`,
        );
        setLoading(false);
        setIsFiltering(false);
        return;
      ...
      // ============================================================
      // Generate CSV from filtered records
      // ============================================================
      const headers = [
        "Employee ID",
        "Employee",
      ...
      if (monthFilter) {
        ...
        reportTitle = `Monthly Report: ${dateObj.toLocaleDateString("en-US", {
          month: "long",
          year: "numeric",
        })} (${selectedEmployees.length} employees)`;
      } else if (dateRangeStart && dateRangeEnd) {
        reportTitle = `Date Range Report: ${new Date(dateRangeStart).toLocaleDateString()} - ${new Date(dateRangeEnd).toLocaleDateString()} (${selectedEmployees.length} employees)`;
      } else if (dateFilter) {
        reportTitle = `Daily Report: ${new Date(dateFilter).toLocaleDateString()} (${selectedEmployees.length} employees)`;
      } else {
        reportTitle = `Full Report (${selectedEmployees.length} employees)`;
      }

      // Build filename
      let filename = "";
      if (monthFilter) {
        filename = `monthly_${monthFilter}_${selectedEmployees.length}_employees.csv`;
      } else if (dateRangeStart && dateRangeEnd) {
        filename = `range_${dateRangeStart}_to_${dateRangeEnd}_${selectedEmployees.length}_employees.csv`;
      } else if (dateFilter) {
        filename = `daily_${dateFilter}_${selectedEmployees.length}_employees.csv`;
      } else {
        filename = `full_${selectedEmployees.length}_employees_${new Date().toISOString().slice(0, 10)}.csv`;
      }
      ...
      filteredRecords.forEach((item) => {
        ...
        if (typeof item.employee === "object" && item.employee !== null) {
          ...
          empDetails = getEmployeeDetails(item.employee.id);
        } else if (typeof item.employee === "number") {
          empDetails = getEmployeeDetails(item.employee);
        ...
        // Get employee name from map or from record
        const employeeName =
          employeeNameMap[empId] ||
          item.employee_name ||
          empDetails?.name ||
          "Unknown";
        ...
        csvData.push(
          [
            `"${empId || "N/A"}"`,
            `"${employeeName}"`,
            `"${empDetails?.company || "N/A"}"`,
            `"${empDetails?.department || "N/A"}"`,
            `"${date}"`,
            `"${formatTimeToAMPM(item.check_in)}"`,
            `"${formatTimeToAMPM(item.check_out)}"`,
            `"${formatDelayTime(item.attendance_delay || item.delay_time)}"`,
    ...
    setLoading,
    setIsFiltering,
    setShowEmployeeSearch,
  ]);

  // ============================================================
  // 4. FILTER CLEAR FUNCTIONS
  // ============================================================
