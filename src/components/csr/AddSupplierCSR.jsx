...
// Helper function to calculate days remaining - returns negative for expired dates
...
// Helper function to calculate status from days remaining
const calculateStatusFromDaysRemaining = (daysRemaining) => {
  if (daysRemaining === "" || daysRemaining === null || daysRemaining === undefined) return "";
...
// Date fields that need auto-calculation
...
const AddSupplierCSR = () => {
  ...
  // Ref for multiple file uploads (additional documents)
  ...
  const [tier2Supplies, setTier2Supplies] = useState([]);
  const [tier3Supplies, setTier3Supplies] = useState([]);
  ...
  // Helper function to get days remaining field name for a given validity field
  const getDaysRemainingField = (validityField) => {
    const mapping = {
      "bsci_validity": "bsci_validity_days_remaining",
      "sedex_validity": "sedex_validity_days_remaining",
      "wrap_validity": "wrap_validity_days_remaining",
      "security_audit_validity": "security_audit_validity_days_remaining",
      "oeko_tex_validity": "oeko_tex_validity_days_remaining",
      "gots_validity": "gots_validity_days_remaining",
      "ocs_validity": "ocs_validity_days_remaining",
      "grs_validity": "grs_validity_days_remaining",
      "rcs_validity": "rcs_validity_days_remaining",
      "iso_9001_validity": "iso_9001_validity_days_remaining",
      "iso_14001_validity": "iso_14001_validity_days_remaining",
      "iso_45001_validity": "iso_45001_validity_days_remaining",
      "trade_license_validity": "trade_license_days_remaining",
      "factory_license_validity": "factory_license_days_remaining",
      "fire_license_validity": "fire_license_days_remaining",
      "membership_validity": "membership_days_remaining",
      "group_insurance_validity": "group_insurance_days_remaining",
      "boiler_license_validity": "boiler_license_days_remaining",
      "berc_license_validity": "berc_days_remaining",
      "drinking_water_license_validity": "drinking_water_license_days_remaining",
    };
    return mapping[validityField] || validityField.replace("_validity", "_validity_days_remaining");
  };

  // Helper function to get status field name for a given validity field
  const getStatusField = (validityField) => {
    const mapping = {
      "bsci_validity": "bsci_status",
      "sedex_validity": "sedex_status",
      "wrap_validity": "wrap_status",
      "security_audit_validity": "security_audit_status",
      "oeko_tex_validity": "oeko_tex_status",
      "gots_validity": "gots_status",
      "ocs_validity": "ocs_status",
      "grs_validity": "grs_status",
      "rcs_validity": "rcs_status",
      "iso_9001_validity": "iso_9001_status",
      "iso_14001_validity": "iso_14001_status",
    };
    return mapping[validityField] || null;
  };

  // Auto-calculate days remaining and status when validity dates change
  useEffect(() => {
    const updates = {};
    
    dateFieldsWithValidity.forEach(validityField => {
      const validityDate = formData[validityField];
      const daysRemainingField = getDaysRemainingField(validityField);
      const statusField = getStatusField(validityField);
      ...
      if (validityDate) {
        ...
        if (statusField) {
          updates[statusField] = calculateStatusFromDaysRemaining(daysRemaining);
        }
      }
    });
    ...
    formData.bsci_validity, formData.sedex_validity, formData.wrap_validity, 
    formData.security_audit_validity, formData.oeko_tex_validity, formData.gots_validity,
    formData.ocs_validity, formData.grs_validity, formData.rcs_validity,
    formData.iso_9001_validity, formData.iso_14001_validity, formData.iso_45001_validity,
    formData.trade_license_validity, formData.factory_license_validity,
    formData.fire_license_validity, formData.membership_validity,
    formData.group_insurance_validity, formData.boiler_license_validity,
    formData.berc_license_validity, formData.drinking_water_license_validity
  ]);
  ...
  const handleClearAllBuildingImages = () => {
    ...
    if (
      window.confirm(
        `Clear all ${buildingImagePreviews.length} selected images?`,
      )
    ) {
  ...
  const handleClearAllFireImages = () => {
    ...
    if (
      window.confirm(`Clear all ${fireImagePreviews.length} selected images?`)
    ) {
  ...
  useEffect(() => {
    const fetchAllChainSupplies = async () => {
      try {
        ...
        setTier2Supplies(tier2Res.data.results || tier2Res.data || []);
        const tier3Res = await getChainSupplies1();
        setTier3Supplies(tier3Res.data.results || tier3Res.data || []);
      } catch (err) {
  ...
  const handleTier2Toggle = (supplierId) => {
    setFormData((prev) => ({
      ...prev,
      tier2_suppliers: prev.tier2_suppliers?.includes(supplierId)
        ? prev.tier2_suppliers.filter((id) => id !== supplierId)
        : [...(prev.tier2_suppliers || []), supplierId],
    }));
  };

  const handleTier3Toggle = (supplierId) => {
    setFormData((prev) => ({
      ...prev,
      tier3_suppliers: prev.tier3_suppliers?.includes(supplierId)
        ? prev.tier3_suppliers.filter((id) => id !== supplierId)
        : [...(prev.tier3_suppliers || []), supplierId],
    }));
  };
  ...
  const createNewChainSupplyHandler = async (isTier2) => {
    ...
    try {
      ...
      if (isTier2) {
        ...
        const res = await getChainSupplies();
        setTier2Supplies(res.data.results || res.data || []);
        const newItem = response.data;
        setFormData((prev) => ({
          ...prev,
          tier2_suppliers: [...(prev.tier2_suppliers || []), newItem.id],
        }));
      } else {
        response = await createChainSupply1(newChainSupply);
        const res = await getChainSupplies1();
        setTier3Supplies(res.data.results || res.data || []);
        const newItem = response.data;
        setFormData((prev) => ({
          ...prev,
          tier3_suppliers: [...(prev.tier3_suppliers || []), newItem.id],
        }));
      }
      ...
      alert("Failed to create new Chain Supply. Please check console.");
    }
  };
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (dateFieldsWithValidity.includes(name)) {
        const daysRemainingField = getDaysRemainingField(name);
        const statusField = getStatusField(name);
        ...
        if (processedValue) {
          ...
          if (statusField) {
            newData[statusField] = calculateStatusFromDaysRemaining(daysRemaining);
          }
        } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        ...
        if (key === "tier2_suppliers" || key === "tier3_suppliers") {
      ...
      // Handle multiple file uploads for additional documents
      for (let i = 1; i <= 4; i++) {
  ...
  // NEW: Render multiple file upload for additional documents
  const renderMultiFileUpload = (label, fieldName, fileInputName) => {
    const currentFiles = multipleFilesRef.current[fieldName] || [];
    ...
    const clearMultiFiles = () => {
      ...
      if (!confirm(`Remove all files from ${label}?`)) return;
      ...
      <div style={formGroupStyle}>
        ...
        <div style={fileInputWrapperStyle}>
          ...
          <label htmlFor={`file-${fieldName}`} style={fileInputLabelStyle}>
            {currentFiles.length > 0 ? `${currentFiles.length} file(s) selected` : 'Choose multiple files'}
          </label>
        </div>
        {currentFiles.length > 0 && (
          <div style={fileListContainerStyle}>
            <div style={fileListHeaderStyle}>
              <span>{currentFiles.length} file(s) selected</span>
              <button type="button" onClick={clearMultiFiles} style={clearAllButtonStyle}>
                Clear All
              </button>
            </div>
            <div style={fileListStyle}>
              {currentFiles.map((file, idx) => (
                <div key={idx} style={fileListItemStyle}>
                  <span style={fileIconStyle}>
                    {file.type?.startsWith('image/') ? '🖼️' : '📄'}
                  </span>
                  <span style={fileListNameStyle}>{file.name}</span>
                  <span style={fileListSizeStyle}>{(file.size / 1024).toFixed(1)} KB</span>
                  <button
                    type="button"
    ...
    return (
      <div style={formGroupStyle}>
        ...
        <Component
          ...
          disabled={isLoading || isDaysRemaining || isStatusField || isReadOnly}
          placeholder={
            isDaysRemaining
        ...
        {isStatusField && value && (
          <div
            style={{
              ...statusHintStyle,
              ...(value === "valid" ? validStatusStyle : expiredStatusStyle),
            }}
          >
            {value === "valid"
              ? "✓ Valid (renewal not needed)"
              : value === "expiring_soon"
  ...
  const renderLicenseGroup = (prefix, label) => (
    <div style={cardStyle}>
      ...
      <div style={cardBodyStyle}>
        <div style={formGridStyle}>
          {renderInput("Validity", `${prefix}_validity`, "date")}
          {renderInput("Days Remaining", `${prefix}_days_remaining`, "number", false, null, true)}
      ...
      <div style={headerStyle}>
        <div style={headerContentStyle}>
          ...
          <div style={titleSectionStyle}>
            <h1 style={titleStyle}>Add New Supplier</h1>
            <p style={subtitleStyle}>
              Complete all sections to add a new supplier/factory
            </p>
          </div>
        </div>
      ...
      <div style={contentWrapperStyle}>
        ...
        <div style={formStyle}>
          <div style={tabContentStyle}>
            ...
            {activeTab === "supplyChain" && (
              <div style={formSectionStyle}>
                <div style={sectionHeaderStyle}>
                  ...
                  <p style={sectionDescriptionStyle}>
                    Manage Tier 2 and Tier 3 suppliers (select or create new)
                  </p>
                </div>
                    ...
                    <div style={cardBodyStyle}>
                      <div style={checkboxListContainerStyle}>
                        ...
                        <div style={checkboxListStyle}>
                          {tier2Supplies.map((cs) => (
                            <div
                              key={cs.id}
                              style={checkboxItemStyle}
                              onClick={() => handleTier2Toggle(cs.id)}
                            >
                              <input
                                type="checkbox"
                                checked={
                                  formData.tier2_suppliers?.includes(cs.id) ||
                                  false
                                }
                                onChange={() => {}}
                                style={checkboxInputStyle}
                              />
                              <div style={checkboxItemContentStyle}>
                          ...
                          {tier2Supplies.length === 0 && (
                            <div style={emptyStateStyle}>
                              No Tier 2 suppliers available. Click "Add New" to
                              create one.
                            </div>
                          )}
                        </div>
                  ...
                  <div style={cardStyle}>
                    ...
                    <div style={cardBodyStyle}>
                      <div style={checkboxListContainerStyle}>
                        ...
                        <div style={checkboxListStyle}>
                          {tier3Supplies.map((cs) => (
                            <div
                              key={cs.id}
                              style={checkboxItemStyle}
                              onClick={() => handleTier3Toggle(cs.id)}
                            >
                              <input
                                type="checkbox"
                                checked={
                                  formData.tier3_suppliers?.includes(cs.id) ||
                                  false
                                }
                                onChange={() => {}}
                                style={checkboxInputStyle}
                          ...
                          {tier3Supplies.length === 0 && (
                            <div style={emptyStateStyle}>
                              No Tier 3 suppliers available. Click "Add New" to
                              create one.
                            </div>
                          )}
                        </div>
                  ...
                  <div style={modalOverlayStyle}>
                    <div style={modalContentStyle}>
                      <h3 style={modalTitleStyle}>
                        Add New {showTier2Modal ? "Tier 2" : "Tier 3"} Supplier
                      </h3>
                      <div style={formGroupStyle}>
                        <label style={labelStyle}>
                      ...
                      <div style={modalActionsStyle}>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            createNewChainSupplyHandler(showTier2Modal)
                          }
            ...
            {activeTab === "certifications" && (
              <div style={formSectionStyle}>
                <div style={sectionHeaderStyle}>
                  ...
                  <p style={sectionDescriptionStyle}>
                    Audit and certification details (Status auto-updates based
                    on validity date)
                  </p>
                </div>
              ...
              <div style={formSectionStyle}>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={cardBodyStyle}>
                    ...
                    {renderChecklistItem(
                      "Wastewater (PH, TDS, BOD, COD, DO, TSS) within limits",
            ...
            {/* UPDATED DOCUMENTS TAB - Now supports multiple file uploads */}
            {activeTab === "documents" && (
              <div style={formSectionStyle}>
                <div style={sectionHeaderStyle}>
                  <h3 style={sectionTitleStyle}>
          ...
          <div style={formActionsStyle}>
            <div style={requiredHintStyle}>
              ...
              <span style={{ marginLeft: "1rem", color: colors.info }}>
                🔄
              </span>{" "}
              Auto-calculated
              <span style={{ marginLeft: "1rem", color: colors.success }}>
                ⚡
              </span>{" "}
              Auto-updated
            </div>
