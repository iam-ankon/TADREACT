...
const StationeryItems = () => {
  ...
  const handleDelete = async (itemId) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      try {
        await stationeryAPI.deleteItem(itemId); // Make sure this method exists in your API
  ...
  const getStats = () => {
    ...
    const inStock = items.filter(
      (i) => getStockStatus(i).label === "In Stock",
    ).length;
    const lowStock = items.filter(
      (i) => getStockStatus(i).label === "Low Stock",
    ).length;
    const outOfStock = items.filter(
      (i) => getStockStatus(i).label === "Out of Stock",
    ).length;
      ...
      <div>
        {filteredItems.length === 0 ? (
          ...
          >
            ...
            >
              Try adjusting your search or filter criteria. You can also add a
      ...
      >
        ...
        >
          <div>
            ...
            >
              {stats.lowStock} items need reorder • {stats.outOfStock} items out
              of stock
            </div>
          </div>
        ...
        >
          Last updated: {new Date().toLocaleTimeString()} • Auto-refreshes every
          5 minutes
        </div>
      </motion.div>
    </div>
