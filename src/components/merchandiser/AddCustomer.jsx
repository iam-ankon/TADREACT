...
import { User, Mail, Phone, MapPin, FileText, Building2, AlertCircle, CheckCircle } from "lucide-react";
...
export default function AddCustomer() {
  ...
  const CUSTOMER_API = "http://119.148.51.38:8000/api/merchandiser/api/customer/";

  // Fetch existing customers for suggestions
  useEffect(() => {
  ...
  const handleCustomerNameChange = (e) => {
    ...
    if (value.length > 1) {
      const filtered = existingCustomers.filter(cust => 
        (cust.customer_name || cust.hrms_customer_name || "").toLowerCase().includes(value.toLowerCase())
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(CUSTOMER_API, postData, {
        headers: {
      ...
      setMessage({ 
        type: "success", 
        text: `Customer "${form.customer_name}" added successfully! It has been synced with HRMS.` 
      });
      ...
      if (err.response) {
        ...
        if (err.response.status === 500) {
          errorText = "Server error. Please contact administrator or check server logs.";
        ...
        errorText = "No response from server. Please check your network connection.";
      } else {
      ...
      <div className="content-container">
        <div className="form-header">
          <div>
            ...
            <p className="page-subtitle">Customer will be synced with HRMS automatically</p>
          </div>
        </div>
          ...
          <div className="form-grid">
            <div className="form-group full-width">
              ...
              <div className="field-hint">
                <small>If this customer exists in HRMS, they will be linked automatically</small>
              </div>
            </div>
