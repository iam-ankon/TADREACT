...
const Sidebar = () => {
  ...
  const hasFullAccess = () => {
    ...
    // USERS WITH FULL ACCESS (including MIZANUR2)
    ...
    // Check mode from user object
    const userStr = localStorage.getItem("user");
    if (userStr) {
      try {
        const user = JSON.parse(userStr);
        if (user.mode === "full_access") {
          return true;
        }
      } catch (e) {
  ...
  const sidebarStyle = {
    ...
    boxShadow:
      "inset -1px 0 0 rgba(0, 0, 0, 0.05), 4px 0 20px rgba(0, 0, 0, 0.04)",
    display: "flex",
    ...
    <div>
      <div style={sidebarStyle}>
        <div style={headerStyle}>
          ...
          <span style={headerTextStyle}>{isOpen ? "Merchandise" : "M"}</span>
        </div>
            ...
            {menuItems.map(({ to, icon, label, badge }) => (
              <li key={to}>
                ...
                >
                  ...
                  {badge && isOpen && <span style={badgeStyle}>{badge}</span>}
