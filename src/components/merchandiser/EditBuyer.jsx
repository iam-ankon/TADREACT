...
export default function EditBuyer() {
  ...
  // In EditBuyer.jsx, update the fetchBuyer function to use row_items:
    ...
    const fetchBuyer = async () => {
      try {
        ...
        setForm({
          ...
          remarks: buyerData.remarks || "",
        });

        // Use row_items from the response
        if (buyerData.rows && buyerData.rows.length > 0) {
          const rows = buyerData.rows.map((row, index) => ({
            id: row.id || Date.now() + index,
            department: row.department || "",
            wgr: row.wgr_number || "", // Map wgr_number to wgr
            item: row.item || "",
            productCategory: row.product_category || "", // Map product_category to productCategory
          }));
          setTableRows(rows);
        } else {
          // If no rows, create one empty row
          setTableRows([
            {
              id: Date.now(),
              department: "",
        ...
        const customerIds = buyerData.customers || buyerData.customer_ids || [];
        const customerIdStrings = customerIds.map((cid) => cid.toString());
        setSelectedCustomers(customerIdStrings);
        setOriginalCustomers([...customerIdStrings]);

        // Also store original data for tracking (optional)
        if (buyerData.rows) {
          setOriginalData({
            departments: buyerData.rows
              .map((r) => r.department)
              .filter(Boolean),
            wgrNumbers: buyerData.rows.map((r) => r.wgr_number).filter(Boolean),
            items: buyerData.rows.map((r) => r.item).filter(Boolean),
            categories: buyerData.rows
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      // Build row_data array with correct field names
      ...
      console.log("📦 row_data for update:", rowData);
      ...
      setError(
        err.response?.data?.message ||
          "Failed to update buyer. Please try again.",
      );
    ...
    return (
      <div style={S.container}>
        ...
        <div style={S.loadingWrap}>
          ...
          <p style={{ color: "#64748b" }}>Loading buyer data...</p>
        </div>
      </div>
    );
  }
      ...
      <div style={S.main}>
        ...
        <div style={S.header}>
          <div style={S.headerLeft}>
            ...
            <div>
              <div style={S.headerTitle}>Edit Buyer</div>
              <div style={S.headerSub}>
                Update buyer information and assignments
              </div>
            </div>
          </div>
