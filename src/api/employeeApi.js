...
// In employeeApi.js - Replace the existing getEmployeeLeaveBalances function

/**
 * Get employee leave balances with pagination support
 * @param {string} employeeId - Optional employee ID (if null, fetches all)
 * @param {boolean} allPages - Whether to fetch all pages
 * @returns {Promise<Object>} - Response with data and pagination
 */
export const getEmployeeLeaveBalances = async (
  ...
  try {
    console.log("🔍 Fetching leave balances. EmployeeId:", employeeId);
    console.log("🔍 All pages mode:", allPages);
    
    // Determine if this is for a specific employee or all employees
    const isSpecificEmployee = employeeId && employeeId !== null && employeeId !== "null" && employeeId !== "";
    ...
    if (allPages) {
      ...
      while (hasMore) {
        // Build URL - different for specific employee vs all employees
        let url;
        if (isSpecificEmployee) {
          url = `employee_leave_balances/?employee_id=${employeeId}&page=${currentPage}&page_size=${pageSize}`;
        } else {
          url = `employee_leave_balances/?page=${currentPage}&page_size=${pageSize}`;
        }
        ...
        if (response.data && response.data.results) {
          ...
          currentPage++;
          console.log(`📊 Page ${currentPage-1}: got ${response.data.results.length} records`);
        } else {
          const data = extractDataFromResponse(response);
      ...
      let url;
      if (isSpecificEmployee) {
        url = `employee_leave_balances/?employee_id=${employeeId}`;
      } else {
        url = `employee_leave_balances/`;
      }
      ...
      let balancesData;
      if (response.data && response.data.results) {
        balancesData = response.data.results;
        console.log(`📊 Got ${balancesData.length} balances from paginated response`);
      } else {
        balancesData = extractDataFromResponse(response);
        console.log(`📊 Got ${balancesData.length} balances from direct response`);
      }
      
      // Log sample data for debugging
      if (balancesData.length > 0) {
        console.log("📋 Sample balance record:", balancesData[0]);
        console.log("📋 Available fields:", Object.keys(balancesData[0]));
        
        // Check if employee_name exists, if not, try to add it from employee object
        if (!balancesData[0].employee_name && balancesData[0].employee) {
          console.log("🔄 Adding employee_name from employee object");
          balancesData = balancesData.map(b => ({
            ...b,
            employee_name: b.employee?.name || b.employee_name
          }));
        }
      } else {
        console.warn("⚠️ No balance records found!");
      }
...
/**
 * Get employee leaves with pagination support and sorting
 * @param {number} page - Page number
 * @param {number} pageSize - Items per page
 * @param {Object|boolean} options - Filter params or allPages flag
 * @param {string} sortBy - Field to sort by (default: 'status_priority')
...
export const getEmployeeLeaves = async (
  ...
  try {
    console.log(`🔍 Getting leaves - Page: ${page}, Size: ${pageSize}`);
    ...
    // If allPages is true, fetch all pages (used for exports)
    if (allPages) {
      let allLeaves = [];
      let currentPage = 1;
      let hasMore = true;
      ...
      while (hasMore) {
        let url = `employee_leaves/?page=${currentPage}&page_size=${fetchPageSize}`;
        ...
        // Add user filtering if not full access
        ...
        console.log(`🌐 Fetching all pages - Page ${currentPage}:`, url);
        const response = await hrmsApi.get(url);
      ...
      return { 
        ...
        pagination: {
          ...
          total_pages: 1,
          current_page: 1,
          page_size: allLeaves.length
        }
      };
    }

    // Single page fetch with pagination
    let url = `employee_leaves/?page=${page}&page_size=${pageSize}`;

    // Add sorting parameters
    if (sortBy === "status_priority") {
      url += `&ordering=status,-start_date`;
    } else {
      const order = sortOrder === "asc" ? "" : "-";
      url += `&ordering=${order}${sortBy}`;
    }
    ...
    if (response.data && response.data.results) {
      ...
      console.log(`📋 Page ${page}: ${leavesData.length} leaves, Total: ${totalCount}, Pages: ${totalPages}`);
...
// employeeApi.js - Replace the existing getMDepartments function

export const getMDepartments = async (page = 1, pageSize = 100, allPages = false) => {
  try {
    if (allPages) {
      let allDepartments = [];
      let currentPage = 1;
      ...
      while (hasMore) {
        const response = await hrmsApi.get(
          `mdepartments/?page=${currentPage}&page_size=${pageSize}`,
        );
      ...
      // FIXED: Use the correct endpoint - mdepartments/
      const response = await hrmsApi.get(
        `mdepartments/?page=${page}&page_size=${pageSize}`,
      );
    ...
    console.error("❌ Error fetching managerial departments:", error);
    return { data: [] };
  }
};
...
export const sendSalaryChangeEmail = async (employeeId, salaryData) => {
  try {
    console.log(`📧 Sending salary change email to employee ${employeeId}`);
    const response = await hrmsApi.post(`employees/${employeeId}/send-salary-email/`, salaryData);
    return response.data;
  } catch (error) {
    console.error("❌ Error sending salary change email:", error);
    throw error;
  }
};
